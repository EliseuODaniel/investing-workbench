"""Persistence for saved research workspace manifests."""
...
class LocalResearchWorkspacesRepository:
    """Store saved research workspace manifests on local disk."""

    def __init__(self, base_dir: Path | str = "research_workspaces") -> None:
        self.base_dir = Path(base_dir)

    def persist_workspace(self, workspace_payload: dict[str, Any]) -> dict[str, Any]:
        """Persist one research workspace manifest."""
    ...
    def list_workspaces(self) -> list[dict[str, Any]]:
        """List saved research workspaces from newest to oldest."""
    ...
    def get_workspace(self, workspace_id: str) -> dict[str, Any]:
        """Load one saved research workspace manifest."""
        manifest_path = self.base_dir / workspace_id / "manifest.json"
        if not manifest_path.exists():
            raise FileNotFoundError(f"Research workspace not found: {workspace_id}")
