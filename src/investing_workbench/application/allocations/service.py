...
from src.investing_workbench.domain.allocations import (
    RebalanceAction,
    RebalanceActionType,
    RebalancePlan,
...
class AllocationPlanningService:
    ...
    def build_plan(self, request: RebalancePlanRequest) -> RebalancePlan:
        ...
        if target_cash + 1e-9 < request.reserve_cash:
            raise ValueError("Target weights leave less cash than the requested reserve_cash floor")
    ...
    def _validate_request(self, request: RebalancePlanRequest) -> None:
        ...
        missing_prices = sorted(asset for asset in required_assets if asset not in request.prices)
        if missing_prices:
            raise ValueError("Missing prices for assets: " + ", ".join(missing_prices))

        invalid_prices = sorted(asset for asset in required_assets if request.prices[asset] <= 0)
        if invalid_prices:
            raise ValueError("Prices must be positive for assets: " + ", ".join(invalid_prices))
