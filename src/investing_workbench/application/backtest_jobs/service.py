"""Application service for asynchronous backtest job execution."""
...
from src.investing_workbench.infrastructure.persistence import LocalBacktestJobsRepository
...
class BacktestJobCancelledError(RuntimeError):
    """Raised when an async backtest job is cancelled before completion."""


class BacktestJobService:
    """Run persisted backtests in the background with observable job status."""

    def __init__(
        self,
        *,
        run_service: RunBacktestService | None = None,
        jobs_repository: LocalBacktestJobsRepository | None = None,
        max_workers: int = 2,
        ...
        self.jobs_repository = jobs_repository or LocalBacktestJobsRepository()
        self.max_workers = max_workers
        if execution_mode not in {"inline", "detached"}:
            raise ValueError("execution_mode must be 'inline' or 'detached'")
        self.execution_mode = execution_mode
        self._autostart = autostart
        self._inline_worker_id = f"inline-worker-{os.getpid()}"
        self._executor = (
            ThreadPoolExecutor(
    ...
    def create_job(self, request: BacktestRunInput | dict[str, Any] | object) -> dict[str, Any]:
        """Persist and enqueue a new async backtest job."""
        run_input = to_backtest_run_input(request)
        now = datetime.now(UTC).isoformat()
        payload = run_input.to_request_payload()
        ...
        job = {
            ...
            "strategy_names": run_input.strategies or [],
        ...
        logger.info("Queued async backtest job %s", job_id)
    ...
    def list_jobs(
        ...
        *,
        ...
        """Return persisted backtest jobs ordered from newest to oldest."""
    ...
    def get_job(self, job_id: str) -> dict[str, Any]:
        """Fetch one backtest job manifest."""
        return self.jobs_repository.get_job(job_id)
    ...
    def cancel_job(self, job_id: str) -> dict[str, Any]:
        ...
        logger.info("Cancellation requested for async backtest job %s", job_id)
        return self.jobs_repository.get_job(job_id)

    def resume_job(self, job_id: str) -> dict[str, Any]:
        """Requeue a failed or cancelled job using the persisted request payload."""
        with self._lock:
            job = self.jobs_repository.get_job(job_id)
            status = str(job.get("status", ""))
            ...
            job.update(
                {
                    ...
                    "progress": self._build_progress(
                        phase="queued",
                        message="Job resumed and queued again.",
                        percent=0.0,
                        updated_at=now,
                    ),
                }
        ...
        logger.info("Resumed async backtest job %s", job_id)
        return self.jobs_repository.get_job(job_id)

    def get_job_response(self, job_id: str) -> dict[str, Any]:
        """Load the persisted run response for a completed job."""
        job = self.jobs_repository.get_job(job_id)
        if str(job.get("status")) != "completed" or not job.get("run_id"):
            raise ValueError(f"Backtest job '{job_id}' does not have a completed run result yet")
        return self.run_service.get_run_response(str(job["run_id"]))

    def get_job_counts(self) -> dict[str, int]:
        """Summarize persisted backtest jobs by status."""
        counts = {
            "queued": 0,
            "running": 0,
            "completed": 0,
    ...
    def latest_job_id(self) -> str | None:
        """Return the newest persisted backtest job id, if any."""
        jobs = self.jobs_repository.list_jobs()
        if not jobs:
            return None
    ...
    def process_next_queued_job(self, *, worker_id: str | None = None) -> dict[str, Any] | None:
        """Claim and process the oldest queued job in the current process."""
    ...
    def run_worker_loop(
        ...
        *,
        ...
        """Process queued jobs in the foreground until the stop condition is met."""
        resolved_worker_id = worker_id or self._build_worker_id()
        processed_jobs = 0
        logger.info(
            "Starting async backtest worker %s in %s mode",
            resolved_worker_id,
            self.execution_mode,
        )
        while True:
    ...
    def _execute_job(
        ...
        *,
        ...
        try:
            ...
            run_id = (
                str(response.run_info["run_id"])
                if response.run_info and response.run_info.get("run_id")
                else None
            )
            self._finish_job(
                job_id,
                status="completed",
                message="Backtest job completed successfully.",
                percent=100.0,
                run_id=run_id,
            )
            logger.info("Async backtest job %s completed with run %s", job_id, run_id)
        except BacktestJobCancelledError:
            self._finish_job(
                job_id,
                status="cancelled",
            ...
            logger.info("Async backtest job %s cancelled", job_id)
        except Exception as exc:  # pragma: no cover - defensive logging path
            logger.exception("Async backtest job %s failed", job_id)
            self._finish_job(
                job_id,
                status="failed",
    ...
    def _start_job(
        ...
        *,
        ...
        if not lock_acquired and not self.jobs_repository.acquire_execution_lock(job_id, worker_id):
            raise RuntimeError(f"Backtest job '{job_id}' is already claimed by another worker")
        with self._lock:
            job = self.jobs_repository.get_job(job_id)
            if str(job.get("status")) == "cancelled" or bool(job.get("cancel_requested")):
                raise BacktestJobCancelledError("Job was cancelled before execution started")
    ...
    def _update_progress(self, job_id: str, payload: dict[str, Any]) -> None:
        with self._lock:
            ...
            if bool(job.get("cancel_requested")):
                raise BacktestJobCancelledError("Job cancellation requested")

            now = datetime.now(UTC).isoformat()
            phase = str(payload.get("phase", "running"))
            message = str(payload.get("message", "Backtest job running."))
    ...
    def _finish_job(
        ...
        *,
        ...
        with self._lock:
            ...
            job["finished_at"] = now
            job["run_id"] = run_id
            job["result_available"] = bool(run_id) and status == "completed"
            job["error"] = error
            job["progress"] = self._build_progress(
                phase=status,
                message=message,
    ...
    def _recover_interrupted_jobs(self) -> None:
        ...
        for job in self.jobs_repository.list_jobs():
            ...
            job["error"] = (
                ...
                else "Job interrupted by process restart before completion. Resume to rerun it."
            )
            job["progress"] = self._build_progress(
                phase="queued" if self._resume_interrupted_jobs else "failed",
                message=(
                    "Job recovered after process restart and queued again."
                    if self._resume_interrupted_jobs
                    else "Job interrupted by process restart before completion."
                ),
        ...
        for job_id in recovered_job_ids:
            logger.info("Recovered interrupted async backtest job %s after process restart", job_id)
            self._enqueue_job(job_id)
    ...
    def _build_worker_id(self) -> str:
        timestamp = datetime.now(UTC).strftime("%Y%m%dT%H%M%SZ")
        return f"worker_{timestamp}_{uuid4().hex[:6]}"
