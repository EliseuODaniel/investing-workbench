...
class TestBenchmarkData:
    ...
    def test_download_market_data_success(self, mock_ticker):
        ...
        # Mock yfinance data
        mock_data = pd.DataFrame({"Close": [100, 105, 102, 108, 110]})
    ...
    def test_create_buy_hold_benchmark(self):
        ...
        price_data = pd.DataFrame({"price": [100, 105, 102, 108, 110]})
    ...
    def test_get_selic_data_caching(self):
        ...
        with patch("src.benchmarks.get_or_create_selic_data") as mock_get_data:
            # Mock SELIC DataFrame
            mock_selic_df = pd.DataFrame({"year": [2023], "month": [1], "rate": [0.01]})
    ...
    def test_create_selic_benchmark_with_caching(self, mock_get_selic_data):
        ...
        # Mock SELIC DataFrame
        mock_selic_df = pd.DataFrame({"year": [2023, 2023], "month": [1, 2], "rate": [0.01, 0.011]})
        mock_get_selic_data.return_value = mock_selic_df
    ...
    def test_calculate_benchmark_metrics(self):
        ...
        equity_curve = pd.DataFrame({"equity": [10000, 10500, 11000, 11500, 12000]}, index=dates)
    ...
    def test_get_selic_benchmark(self, mock_get_monthly_rate):
        ...
        result = get_selic_benchmark(
            start_date="2023-01-01", end_date="2023-01-31", initial_capital=10000
        )
...
class TestBenchmarkConfig:
    ...
    def test_benchmark_config_creation(self):
        ...
        config = BenchmarkConfig(ticker="^BVSP", name="IBOVESPA", enabled=True)
    ...
    def test_benchmark_config_defaults(self):
        ...
        config = BenchmarkConfig(ticker="SPY", name="S&P 500")
    ...
    def test_app_config_with_benchmarks(self):
        ...
        benchmarks = [BenchmarkConfig("^BVSP", "IBOVESPA", True)]

        backtest_config = BacktestConfig(benchmarks=benchmarks, include_selic_benchmark=True)

        strategy = StrategyConfig(
            name="Test Strategy", class_path="strategies.test.TestStrategy", parameters={}
        )

        app_config = AppConfig(backtest=backtest_config, strategies=[strategy])
