...
export function buildResultsInterpretation(
  ...
  const executionConstrained = Object.entries(results).filter(([, result]) => {
    return result.execution_summary?.liquidity_constrained;
  });
  if (executionConstrained.length > 0) {
    const names = executionConstrained.map(([name]) => name).join(', ');
    insights.push({
      tone: 'caution',
      title: 'Liquidez interferiu na execucao',
      body: `${names} registraram fills parciais ou ordens rejeitadas. Vale ler o resumo de execucao antes de confiar no retorno final.`,
    });
  }
